...
roc_results <- list()
auc_results <- data.frame(biomarker=character(), biomarker_name=character(), auc=numeric(), ci_lower=numeric(), ci_upper=numeric(), p_value=numeric(), n_total=numeric(), n_events=numeric(), stringsAsFactors=FALSE)
for (biomarker in roc_biomarkers) {
  ...
  auc_results <- rbind(auc_results, data.frame(biomarker=biomarker, biomarker_name=biomarker_names_roc[[biomarker]], auc=as.numeric(auc_ci[2]), ci_lower=as.numeric(auc_ci[1]), ci_upper=as.numeric(auc_ci[3]), p_value=NA, n_total=length(actual_outcome), n_events=sum(actual_outcome==1, na.rm=TRUE), stringsAsFactors=FALSE))
}
